...
# Using lambda,zip and map
def generate_deck_using_lambda_zip_map(suits: 'list', value: 'list') -> 'list':
    """Function which generates list of 52 card pairs 
    used in Poker game"""
    def pattern(x, y):
        return list(map(lambda sub: sub[0]+sub[1], zip(y, [x]*len(y))))
    deck = [j for i in list(
        map(lambda _x:pattern(_x, value), suits)) for j in i]
    return deck


# Using Normal Function
def generate_deck_normal(suits, value, mydeck=None):
    '''
...
def process(set1: list) -> tuple:
    ...
    if color_check == True:
        ...
        if sequence_check == True:
            ...
            duplicate_count = {k: v for k, v in sorted(
                duplicate_count.items(), key=lambda item: item[1], reverse=True)}
